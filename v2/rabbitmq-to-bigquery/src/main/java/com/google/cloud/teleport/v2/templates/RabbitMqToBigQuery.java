...
/**
 * The {@link RabbitMqToBigQuery} pipeline is a streaming pipeline which ingests text data from Kafka,
 ...
 * </pre>
 */
public class RabbitMqToBigQuery {

  /* Logger for class. */
  private static final Logger LOG = LoggerFactory.getLogger(RabbitMqToBigQuery.class);
  ...
  public interface Options extends PipelineOptions {
    ...
    void setRabbitUser(String value);

    @Description("Rabbit password")
    @Default.String("password")
    String getRabbitPassword();
    void setRabbitPassword(String value);

    @Description("Rabbit host")
    @Default.String("35.226.28.191")
    String getRabbitHost();
    void setRabbitHost(String value);

    @Description("Rabbit virtual host")
    @Default.String("%2f")
    String getRabbitVirtualHost();
    void setRabbitVirtualHost(String value);

    @Description("Rabbit port")
    @Default.String("5672")
    String getRabbitPort();
    void setRabbitPort(String value);

    @Description("Rabbit queue name")
    @Default.String("cart.idle.process.dataflow")
    String getRabbitQueue();
    void setRabbitQueue(String value);

    @Description("Rabbit queue read max records")
    @Default.Long(100)
    Long getRabbitMaxReadRecords();
    void setRabbitMaxReadRecords(Long value);

    @Description("Google Cloud Storage bucket to save file to trigger new dataflow job")
    @Default.String("gs://rabbitmq-dataflow/output")
    String getBucketTrigger();
    void setBucketTrigger(String value);
   ...
   * RabbitMqToBigQuery#run(Options)} method to start the pipeline and invoke {@code
  ...
  public static PipelineResult run(Options options) {
    ...
    String sURI = "amqp://" 
                + options.getRabbitUser() + ":" 
                + options.getRabbitPassword() + "@" 
                + options.getRabbitHost() + ":" 
                + options.getRabbitPort() + "/" 
                + options.getRabbitVirtualHost();
     */
    //String sURI = "amqp://T00199:Livelo%402020@rabbitmq-transactional.k8s.dev.pontoslivelo.com.br:30600/%2f";
    String sURI = "amqp://T00199:Livelo%402020@10.150.101.131:30600/%2f";
    LOG.info(sURI);
    ...
    PCollection<RabbitMqMessage> messages = pipeline
        .apply("Read Rabbit queue",
            ...
            .withMaxNumRecords(options.getRabbitMaxReadRecords())
        );

    final TupleTag<String> dummyTag = new TupleTag<String>(){};

    PCollection<KV<String,String>> rowsProcessed = messages.apply("Transform RabbitMqMessage to Json",
    ...
    PCollectionTuple convertedTableRows =
        rowsProcessed.apply("ConvertMessageToTableRow", new MessageToTableRow(options));
     ...
     */
    WriteResult failedTransformResult = PCollectionList.of(convertedTableRows.get(UDF_DEADLETTER_OUT))
     ...
     */
    WriteResult failedInsertsResult = failedInserts.apply(
    ...
    /*
    messages.apply("Wait write on BigQuery", 
                Wait.on(writeResult.getFailedInsertsWithErr()))
            .apply("Map RabbitMqMessage to String", 
                ParDo.of(new DoFn<RabbitMqMessage, String>() {
        ...
        })).apply("Write results on GCS", 
            TextIO.write()
                  .to(String.format("%s/%s", "gs://rabbitmq-dataflow/output", fileTimestamp))
        );
    
    PCollectionList<BigQueryInsertError> waitList = PCollectionList.of(writeResult.getFailedInsertsWithErr())
                                                    .and(failedTransformResult.getFailedInsertsWithErr())
                                                    .and(failedInsertsResult.getFailedInsertsWithErr());
    */
    pipeline.apply("Create trigger file", Create.of(fileTimestamp))
            .apply("Wait write on BigQuery", Wait.on(writeResult.getFailedInsertsWithErr()))
            .apply("Wait write Transform Failures", Wait.on(failedTransformResult.getFailedInserts()))
            .apply("Wait write Insert Failures", Wait.on(failedInsertsResult.getFailedInserts()))
            .apply("Write trigger on GCS", 
                TextIO.write()
                    .to(String.format("%s/%s", options.getBucketTrigger(), fileTimestamp))
        );
