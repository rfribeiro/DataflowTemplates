...
import com.google.cloud.teleport.v2.templates.RabbitMqToBigQuery.MessageToTableRow;
...
public class RabbitMqToBigQueryTest {
  ...
  /** Tests the {@link RabbitMQToBigQuery} pipeline end-to-end. */
  @Test
  public void testRabbitMqToBigQueryE2E() throws Exception {
    // Test input
    ...
    RabbitMqToBigQuery.Options options =
            PipelineOptionsFactory.create().as(RabbitMqToBigQuery.Options.class);
    ...
    // Assert
    PAssert.that(transformOut.get(RabbitMqToBigQuery.UDF_DEADLETTER_OUT)).empty();
    PAssert.that(transformOut.get(RabbitMqToBigQuery.TRANSFORM_DEADLETTER_OUT)).empty();
    PAssert.that(transformOut.get(RabbitMqToBigQuery.TRANSFORM_OUT))
            .satisfies(
                    collection -> {
    ...
    // Assert
    PAssert.that(badTransformOut.get(RabbitMqToBigQuery.UDF_DEADLETTER_OUT))
            .satisfies(
                    collection -> {
    ...
    PAssert.that(badTransformOut.get(RabbitMqToBigQuery.TRANSFORM_DEADLETTER_OUT)).empty();
    PAssert.that(badTransformOut.get(RabbitMqToBigQuery.TRANSFORM_OUT)).empty();
